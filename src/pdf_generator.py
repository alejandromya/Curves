...
def generar_tabla_ciclos(detalles_ciclos, df, fuerza_max, deformacion_max):
    ...
    # Dciclico
    Dciclico = ultimo_ciclo["deform_high_end"] - primer_ciclo["deform_high_start"]
    ...
    fila.extend([f"{f2mm_val:.2f}", f"{f3mm_val:.2f}", f"{df_max_rel:.4f}", f"{Dciclico:.4f}"])
